...
/// Corrected solar transit time in universal time.
///
/// Equation from Astronomical Algorithms page 102.
...
/// Corrected hour angle for a given solar angle.
///
/// Equation from Astronomical Algorithms page 102.
pub fn corrected_hour_angle(
    ...
    after_transit: bool,
    sidereal_time: Angle,
    right_ascension: InterpolatedAngle,
...
pub(crate) struct InterpolatedAngle {
    pub(crate) current: Angle,
    previous: Angle,
    next: Angle,
}

impl InterpolatedAngle {
    pub(crate) fn new(current: Angle, previous: Angle, next: Angle) -> Self {
        Self {
            current,
            previous,
            next,
    ...
    fn interpolated(&self, factor: f64) -> Angle {
        interpolate_angles(self.current, self.previous, self.next, factor)
    }

    fn interpolated_value(&self, factor: f64) -> f64 {
        interpolate(
            self.current.degrees,
            self.previous.degrees,
            self.next.degrees,
            factor,
        )
    }
...
///
/// # Note
/// Casting to `i32` is intentionally done for decimal truncation.
...
mod tests {
    ...
    fn calculate_julian_century() {
        ...
        assert!((julian_century - (-0.072_183_436_002_737_86)).abs() < 1e-15);
    }
    ...
    fn calculate_mean_solar_longitude() {
        ...
        assert!((mean_solar_longitude.degrees - 201.80719320670732).abs() < 1e-12);
    }
    ...
    fn calculate_apparent_solar_longitude() {
        ...
        assert!((apparent_solar_longitude - 3.489_069_182_045_206).abs() < 1e-12);
    }
    ...
    fn calculate_mean_obliquity_of_the_ecliptic() {
        ...
        assert!((mean_obliq_of_ecliptic.degrees - 23.440229684413012).abs() < 1e-12);
    }
    ...
    fn calculate_apparent_obliquity_of_the_ecliptic() {
        ...
        assert!((apparent_obliq_of_ecliptic.degrees - 23.43999110619955).abs() < 1e-12);
    }
    ...
    fn calculate_mean_solar_anomaly() {
        ...
        assert!((mean_solar_anomaly.degrees - 278.993_966_431_597_5).abs() < 1e-12);
    }
    ...
    fn calculate_solar_equation_of_the_center() {
        ...
        assert!((solar_equation_of_center.degrees - (-1.897323843371985)).abs() < 1e-12);
    }
    ...
    fn calculate_mean_lunar_longitude() {
        ...
        assert!((mean_lunar_longitude.degrees - 38.747190008209145).abs() < 1e-12);
    }
    ...
    fn calculate_acending_lunar_node_longitude() {
        ...
        assert!((ascending_lunar_node.degrees - 264.657131805429).abs() < 1e-12);
    }
    ...
    fn calculate_mean_sidereal_time() {
        ...
        assert!((mean_sidereal_time.degrees - 21.801339167752303).abs() < 1e-12);
    }
    ...
    fn calculate_nutation_longitude() {
        ...
        assert!((nutation_longitude - 0.0044525358169686564).abs() < 1e-15);
    }
    ...
    fn calculate_nutation_in_obliquity() {
        ...
        assert!((nutation_obliq - (-0.000_092_747_500_292_341_56)).abs() < 1e-15);
    }
    ...
    fn calculate_altitude_of_celestial_body() {
        ...
        assert!((celestial_body.degrees - (-0.900_615_621_559_432_1)).abs() < 1e-12);
    }
}
