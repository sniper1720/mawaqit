...
fn main() {
    ...
    let resolved = PolarEstimation::NearestLatitude.resolve_latitude(dt, tromso, Madhab::Shafi);
    ...
    let params = Configuration::new(18.0, 17.0)
        ...
        .done();
    let summer_times = PrayerTimes::try_new(date, tromso, params).unwrap();
    println!("\nNearestLatitude via API:");
    println!(
        "  {:?} {:?} {:?} {:?} {:?} {:?}",
        summer_times.time(Prayer::Fajr),
        summer_times.time(Prayer::Sunrise),
        summer_times.time(Prayer::Dhuhr),
        summer_times.time(Prayer::Asr),
        summer_times.time(Prayer::Maghrib),
        summer_times.time(Prayer::Isha),
    );

    // Also for Jan 15 (winter)
    let winter = NaiveDate::from_ymd_opt(2026, 1, 15).expect("valid date");
    let winter_midnight: DateTime<Utc> = winter.and_hms_opt(0, 0, 0).unwrap().and_utc();
    println!("\n--- Winter (Jan 15) ---");
    times_at(70.0, "Original 70°", winter, tromso);

    let winter_resolved =
        PolarEstimation::NearestLatitude.resolve_latitude(winter_midnight, tromso, Madhab::Shafi);
    ...
    let winter_times = PrayerTimes::try_new(winter, tromso, params).unwrap();
    println!("\nWinter NearestLatitude via API:");
    println!(
        "  {:?} {:?} {:?} {:?} {:?} {:?}",
        winter_times.time(Prayer::Fajr),
        winter_times.time(Prayer::Sunrise),
        winter_times.time(Prayer::Dhuhr),
        winter_times.time(Prayer::Asr),
        winter_times.time(Prayer::Maghrib),
        winter_times.time(Prayer::Isha),
    );
}
